var projects = [{
    ...
    },
    {
        ...
        "title": "Halcyon",
        "width": 640,
        "description": "A non profit mental health organization called Halcyon to improve wait times for mental health patients using mobile apps.<br><br>Myself and 12 other friends from the SHAD Valley Program have been working for the past few months do develop a solution which includes the following: <br><br>1) Self care plan<br>2) Personalized specialists and emergency department suggestions<br>3) Emergency hotlines<br>4) Anonymous group support sorum<br>5) Treatment and medication information",
        "thumbnail": "halcyon.jpg",
        "images": ['halcyon.jpg'],
        "languages": ['HTML/CSS/JS/PHP - Web Client', 'Swift 3 - iOS', 'Java - Android']
    },
    {
        "id": 3,
    ...
    {
        "id": 4,
        "title": "BridgeDay Conference Website",
        "width": 640,
    ...
    {
        "id": 5,
        "title": "Balls iOS Game",
        "width": 640,
        "description": "A new obsessive trend spread across my school - <a href='https://itunes.apple.com/us/app/ballz/id1139609950?mt=8' target='_blank'>Ballz</a>. With a 4 day weekend approaching, I decided to try and recreate it. <br><br> The final product turned out much better than I expected - the game was fully functional with a single ball. In the above picture, my version is on the left and the real game is on the right.",
        "thumbnail": "balls.jpg",
        "images": ['balls.jpg', 'balls2.png'],
        "languages": ['Swift 3']
    },
    {
        "id": 6,
        "title": "TU20 Website",
        "width": 640,
        "description": "As the Web Developer for TechUnderTwenty - a non-profit youth organization, I have been developing a new website for us as our current website is made with a template. <br><br>Expected launch: June 2017",
        "thumbnail": "tu20.jpg",
        "images": ['tu20.jpg'],
        "languages": ['HTML', 'CSS', 'JS', 'SQL', 'jQuery']
    },
    {
        "id": 7,
    ...
    {
        "id": 8,
        "title": "IRHS Announcements Page",
        "width": 640,
        "description": "As my school's <a href='http://irs.hdsb.ca/announcements/' target='_blank'>announcements page</a> is ugly (second picture above), I decided to create an alternate announcements page for our school. I used a cross-origin GET request with AJAX and then parsed the response to create a sligtly more aesthetically pleasing version of our announcements page.",
        "thumbnail": "irhsann.jpg",
        "images": ['irhsann.jpg', 'irhsann2.jpg'],
        "languages": ['HTML', 'CSS', 'JS', 'jQuery', 'AJAX']
    },
    {
        "id": 9,
        "title": "Pinch Perfect",
    ...
    {
        "id": 10,
        "title": "Running Goat iOS Game",
        "width": 640,
        "description": "My first iOS App - a simple goat game. Avoid the walls by tapping on the screen to flip the goat. <br><br><a href='https://www.youtube.com/watch?v=hLOhirsXx9c' target='_blank'>Tutorial followed</a>",
        "thumbnail": "goat.jpg",
        "images": ['goat.jpg', 'goat2.jpg', 'goat3.jpg'],
        "languages": ['Swift 3']
    },
    {
        "id": 11,
        "title": "Turing Chess Game",
        "width": 640,
    ...
    {
        "id": 12,
        "title": "Wheel of Python",
        "width": 640,
        "description": "Project for Grade 12 Computer Science. Created a wheel spinning program using Python Tkinter to simulate the spin of a wheel.",
        "thumbnail": "wheelofpython.jpg",
        "images": ['wheelofpython.jpg'],
        "languages": ['Python']
    },
    {
        "id": 13,
        "title": "EnTech IRHS Proposal Website",
        "width": 640,
        "description": "Created a supplementary website for my school's <a href='http://www.oce-ontario.org/programs/commercialization-programs/advancingeducation-program' target='_blank'>OCE Advancing Education proposal.</a>",
        "thumbnail": "thinktank.jpg",
        "images": ['thinktank.jpg'],
        "languages": ['HTML', 'CSS', 'JS', 'jQuery']
    },
    {
        "id": 14,
        "title": "TU20 Cup Event Website",
        "width": 640,
        "description": "Event website for the TechUnderTwenty Cup, featuring a schedule and response form.",
        "thumbnail": "cup.jpg",
        "images": ['cup.jpg'],
        "languages": ['HTML', 'CSS', 'JS', 'jQuery']
    }
...
$(document).ready(function() {
    ...
    for (var i = 0; i < projects.length; i++) {
        ...
        toAppend = '<div class="mdl-cell mdl-cell--4-col"><div project="' + project['id'] + '" class="mdl-card-square mdl-card">';
        toAppend += '<img src="img/thumbnails/' + project['thumbnail'] + '" class="projectImg">';
        toAppend += '<center><figcaption>' + project['title'] + '</p></center>';
        toAppend += '</div></div>';

        $('.mdl-container').append(toAppend);
    }

    $('.mdl-card').on('click', function(e) {
        document.location.href = '#modal';
        num = $(e.currentTarget).attr('project');
        projectClicked = projects[num];

        $('.remodal').empty();

        toAppend = '<center><button data-remodal-action="close" class="remodal-close"></button>';
        toAppend += '<h1 style="font-size: 45px; font-family: Equal;">' + projectClicked['title'] + '</h1>';

        if (projectClicked.images.length != 0) {
            toAppend += ' <div class="csslider1 no-autoplay" style="max-width:' + projectClicked['width'] + 'px;">';

            for (var i = 0; i < projectClicked.images.length; i++) {
                toAppend += '<input name="cs_anchor1" id="cs_slide1_' + i + '" type="radio" class="cs_anchor slide"';
                if (i == 0) {
                    toAppend += ' checked=""';
                }
                toAppend += '>';
            ...
            for (var i = 0; i < projectClicked.images.length; i++) {
                if (i == 0) {
                    toAppend += '<li class="cs_skeleton"><img src="cssslider_files/csss_images1/' + projectClicked.images[i] + '" style="width: 100%;"></li>';
                }
                toAppend += '<li class="num' + i + ' img slide"> <img src="cssslider_files/csss_images1/' + projectClicked.images[i] + '" alt="' + projectClicked.images[i].slice(0, -4) + '" title="' + projectClicked.images[i].slice(0, -4) + '"></li>';
            }

            toAppend += '</ul><div class="cs_arrowprev">';

            for (var i = 0; i < projectClicked.images.length; i++) {
                toAppend += '<label class="num' + i + '" for="cs_slide1_' + i + '"><span><i></i><b></b></span></label>';
            }
        ...
        toAppend += '</center><div class="langs">Languages: ' + projectClicked.languages[0];

        for (var i = 1; i < projectClicked.languages.length; i++) {
          toAppend +=  ', ' + projectClicked.languages[i];
        }
        toAppend += '</div><br>';

                toAppend += '<p class="projectDesc">' + projectClicked['description'] + '</p>';

        toAppend += '<button data-remodal-action="confirm" class="remodal-confirm">O K</button>';
        $('.remodal').append(toAppend);
