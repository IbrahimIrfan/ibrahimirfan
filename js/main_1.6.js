...
$(document).ready(function() {
    ...
    $("#downArrow").hover(function() {
        ...
        if (document.location.href = "#modal", num = $(e.currentTarget).attr("project"), projectClicked = projects[num], $(".remodal").empty(), toAppend = '<center><button data-remodal-action="close" class="remodal-close"></button>', toAppend += '<h1 style="font-size: 45px; font-family: Equal;">' + projectClicked.title + "</h1>", 0 != projectClicked.images.length) {
        ...
        $(".mdl-grid").empty();
        for (var t = $(e.currentTarget).attr("type"), a = 0; a < projects.length; a++)
            if (project = projects[a], project.type == t) {
                var o = '<div class="mdl-cell mdl-cell--4-col"><div project="' + project.id + '" class="mdl-card-square mdl-card">';
                o += '<img src="img/thumbnails/' + project.thumbnail + '" class="projectImg" alt="' + project.title + '">', o += "<center><figcaption>" + project.title + "</figcaption></center>", o += "</div></div>", $(".mdl-grid").append(o)
            }
